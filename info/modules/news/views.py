...
def followed_user():
    ...
    user = g.user
    if not user:
        return jsonify(error=RET.SESSIONERR, errmsg='未登录')
    ...
    if not other:
        return jsonify(error=RET.NODATA, errmsg='未查到数据')
    # 4.根据要执行的操作去修改对应的数据
    if action == 'follow':
...
def news_detail(news_id):
    ...
    if g.user:
        try:
            ...
            # 1. 查询出当前新闻的所有评论 ([COMMENT]) 取到所有的评论id  [1, 2
